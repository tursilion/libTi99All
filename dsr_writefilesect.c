...
unsigned char writefilesect(const char *szPath, unsigned int vdppath, unsigned int vdpdata, unsigned int start, unsigned int count, char *buf) {
    ...
    // set the unit
    if (buf == 0) buf=(unsigned char*)SBR_DEFAULT_INFO_ADR;
    ...
    // If new file, the rest of the file info struct must already be filled in

    unsigned char code = getsbrbase(szPath) | SBR_FILESECTORWRITE;
...
#else 
unsigned char writefilesect(const char *szPath, unsigned int vdp) { return DSR_ERR_DSRNOTFOUND; }
